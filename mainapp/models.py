...
class LatestProductsManager:
    ...
    def get_products_for_main_page(self, *args, **kwargs):
        ...
        """Метод который возвращает на стартовую страницу последние
        добавленные товары"""
        ...
        for ct_model in ct_models:
            model_products = ct_model.model_class()._base_manager.\
                                 all().order_by('-id')[:5]
        ...
        if with_respect_to:
            ...
            if ct_model.exists():
                if with_respect_to in args:
                    return sorted(
                        products, key=lambda x: x.__class__._meta.\
...
class Product(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if img.height < min_height or img.width < min_width:
            raise MaxRsolutionErrorException('Изображение слишком маленькое')
        if img.height > max_height or img.width > max_width:
            raise MaxRsolutionErrorException('Изображение слишком большое')
        return image
...
# Ниже представлены модели товаров для магазина с перечислением
# опциональных полей
...
class Smartphone(Product):
    ...
    resolution = models.CharField(max_length=255, verbose_name='Разрешение '
                                                               'экрана')
    accum_volume = models.CharField(max_length=255, verbose_name='Объем '
                                                                 'батареи')
    ram = models.CharField(max_length=255,
                           verbose_name='Оперативная память')
    ...
    sd_volume_max = models.CharField(max_length=255,
                                     verbose_name='Максимальный объем '
                                                  'встраиваимой памяти')
    main_cam_mp = models.CharField(max_length=255, verbose_name='Главная'
                                                                ' камера')
